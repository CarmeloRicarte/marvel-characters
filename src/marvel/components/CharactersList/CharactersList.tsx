...
export const CharactersList: React.FC = () => {
  ...
  return (
    <>
      ...
      <Spinner
        visibility={isLoading && !registerToSearch ? 'visible' : 'hidden'}
      />
      {!isLoading &&
        ((characters.length === 0 && searchedCharacterResults.length === 0) ||
          (hasSearchedByName && searchedCharacterResults.length === 0)) && (
          <div>No Results</div>
        )}
            ...
            <div className={styles.charactersList}>
              {characters.map((character) => {
                const cardData = {
                  ...
                  imageData: character?.thumbnail ?? {
                    path: undefined,
                    extension: undefined,
                  },
                  linkToMoreInfo:
                    character?.urls?.find((url) => url.type === 'comiclink')
                      ?.url ?? '', // setted comiclink because detail and wiki urls are not working properly in the Marvel API
                };

                return (
                  <CharacterCard key={character.id} character={cardData} />
                );
              })}
            </div>
      ...
      {searchedCharacterResults.length > 0 && (
        <>
          ...
          <div className={styles.pagination}>
            <Pagination
              isDisabled={
                isLoading ? true : !areMoreCharactersSearchedAvailable
              }
